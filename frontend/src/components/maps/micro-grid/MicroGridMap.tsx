...
import {
  ...
  createGlowLayer,
  createHouseLayer,
  createHouseGlowLayer,
} from "./layers/microGridLayers";
...
export function MicroGridMap() {
  ...
  const [gridHoverInfo, setGridHoverInfo] = useState<{
    properties: any;
    x: number;
    y: number;
  } | null>(null);
  const [searchQuery, setSearchQuery] = useState("");
  const [filterType, setFilterType] = useState<"all" | "producer" | "consumer">(
    "all",
  );
  const [viewState, setViewState] = usePersistedViewState("micro-grid", {
    longitude: MICROGRID_CENTER.lon,
    latitude: MICROGRID_CENTER.lat,
    zoom: 15,
    pitch: 0,
    bearing: 0,
  });
  ...
  const { mapStyle, toggle, isSatellite } = useMapStyle("dark");
  const { infrastructure } = useElectricalGridData(getApiUrl);
  const [gridMode, setGridMode] = useState<"grid-tied" | "islanded">(
    "grid-tied",
  );
  ...
  const [apiFeeders, setApiFeeders] = useState<any>({
    type: "FeatureCollection",
    features: [],
  });
  const [apiElecStatus, setApiElecStatus] = useState<any>(null);

  // Microgrid data now relies on fallbacks or WebSocket stream
  useEffect(() => {
    // fetchMicrogridData removed to avoid 502 errors
  }, []);

  // Stats
  const totalGen = meters.reduce((s, m) => s + m.gen_kwh, 0);
  const totalCons = meters.reduce((s, m) => s + m.cons_kwh, 0);
  const avgVolt = meters.length
    ? meters.reduce((s, m) => s + m.voltage_v, 0) / meters.length
    : 230;
          ...
          setMeters((prev) =>
            prev.map((m) => {
              ...
              if (r)
                return {
                  ...m,
                  gen_kwh: r.energy_generated || 0,
                  cons_kwh: r.energy_consumed || 0,
                  voltage_v: r.voltage || 230,
                };
              return m;
            }),
          );
        }
        if (d.type === "grid_status") {
          setGridStatus(d.data);
        }
      } catch {
        /* silent */
      }
  ...
  const houseGlowLayer = useMemo(
    () => createHouseGlowLayer(animTime),
    [animTime],
  );
  ...
  const filtered = useMemo(() => {
    return filterMetersInBoundary(meters).filter((m) => {
      const matchSearch =
        ...
        m.meter_type.toLowerCase().includes(searchQuery.toLowerCase());
      const isProducer = m.gen_kwh > m.cons_kwh;
      const matchFilter =
        filterType === "all" ||
        (filterType === "producer" && isProducer) ||
        (filterType === "consumer" && !isProducer);
  ...
  return (
    <div className="h-full w-full relative bg-slate-950">
      ...
      <div className="absolute top-0 left-0 right-0 z-[1000] bg-gradient-to-b from-black/80 to-transparent p-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-white font-bold text-lg">Island Grid</h2>
          </div>
          <div className="flex items-center gap-2">
            {lastRefresh && (
      ...
      <div className="absolute top-16 left-4 z-[1000] flex items-center gap-2">
        <div className="glass px-4 py-2 rounded-xl flex items-center gap-6 shadow-[0_8px_32px_rgba(0,0,0,0.4)] border-white/5 backdrop-blur-2xl">
          ...
          <div className="hidden md:flex items-center gap-3">
            ...
            <div className="flex flex-col">
              <span className="text-white text-[13px] font-black tracking-tight leading-none">
                {gridStatus
                  ? Math.min(
                      gridStatus.load_mw,
                      gridStatus.circuit_limit_mw,
                    ).toFixed(2)
                  : "0.00"}{" "}
                <span className="text-slate-500 text-[10px]">MW</span>
              </span>
          ...
          <div className="hidden lg:flex items-center gap-3">
            ...
            <div className="flex flex-col">
              <span className="text-white text-[13px] font-black tracking-tight leading-none">
                {gridStatus?.diesel?.power_mw?.toFixed(2) ?? "0.00"}{" "}
                <span className="text-slate-500 text-[10px]">MW</span>
              </span>
              <span className="text-[9px] text-slate-500 font-bold uppercase mt-1 tracking-wider">
                Diesel Gen
          ...
          {/* Distributed Stats */}
          <div className="hidden xl:flex items-center gap-4 border-l border-white/5 pl-4">
            <div className="flex flex-col">
              <span className="text-emerald-400 text-[11px] font-bold tracking-tight leading-none">
                {(totalGen / 1000).toFixed(2)} MW
              </span>
            ...
            <div className="flex flex-col">
              <span className="text-rose-400 text-[11px] font-bold tracking-tight leading-none">
                {(totalCons / 1000).toFixed(2)} MW
              </span>
              <span className="text-[8px] text-slate-500 font-bold uppercase mt-1">
                Load
              </span>
        ...
        onMouseMove={(e) => {
          try {
            ...
            if (masterFeatures.length > 0) {
              setGridHoverInfo({
                properties: masterFeatures[0].properties,
                x: e.point.x,
                y: e.point.y,
              });
              setHoverInfo(null);
            ...
            if (features.length > 0 && features[0].geometry?.type === "Point") {
              const coords = (features[0].geometry as GeoJSON.Point)
                .coordinates;
              const meter = filtered.find(
                (m) =>
                  Math.abs(m.lon - coords[0]) < 0.001 &&
                  Math.abs(m.lat - coords[1]) < 0.001,
              );
      ...
      {showLegend && (
        <div className="absolute bottom-24 left-4 z-[1000] glass rounded-xl p-3 w-48 shadow-2xl text-[10px]">
          ...
          <p className="text-slate-400 font-bold uppercase tracking-widest mt-3 mb-2">
            Line Networks
          </p>
          {[
            ...
            <div key={name} className="flex items-center gap-2 mb-1.5">
              <div
                className="w-8 h-0 border-t shrink-0"
                style={{
                  borderColor: color as string,
                  borderTopWidth: (style as string).split(" ")[0],
                  borderStyle: (style as string).includes("dashed")
                    ? "dashed"
                    : "solid",
                }}
              />
          ...
          {[
            ...
            <div key={name} className="flex items-center gap-2 mb-1.5">
              <div
                className="w-8 h-0 border-t-2 shrink-0"
        ...
        >
          <div className="glass px-3 py-2 rounded-lg shadow-xl min-w-[180px]">
            <div className="flex items-center gap-2 mb-1">
              {(() => {
                const Icon = METER_ICONS[hoverInfo.meter.meter_type] || Zap;
            ...
            <div className="text-[10px] text-slate-400">
              {hoverInfo.meter.meter_type.replace("_", " ")}
            </div>
            <div className="mt-1 grid grid-cols-2 gap-x-3 gap-y-0.5">
              <span className="text-slate-500">Gen</span>
              <span className="text-right text-[10px] font-bold text-emerald-400">
                {hoverInfo.meter.gen_kwh.toFixed(2)} kWh
              </span>
              <span className="text-slate-500">Cons</span>
              <span className="text-right text-[10px] font-bold text-amber-400">
                {hoverInfo.meter.cons_kwh.toFixed(2)} kWh
              </span>
              <span className="text-slate-500">Net</span>
              <span
                className={`text-right text-[10px] font-bold ${hoverInfo.meter.gen_kwh > hoverInfo.meter.cons_kwh ? "text-emerald-400" : "text-rose-400"}`}
              >
                {(hoverInfo.meter.gen_kwh - hoverInfo.meter.cons_kwh).toFixed(
                  2,
                )}{" "}
                kWh
              </span>
              <span className="text-slate-500">Volt</span>
              <span className="text-right text-[10px] font-bold text-blue-400">
                {hoverInfo.meter.voltage_v.toFixed(0)} V
              </span>
            </div>
          </div>
        </div>
      ...
      {gridHoverInfo && (
        ...
        >
          <div className="glass px-3 py-2 rounded-lg shadow-xl min-w-[220px] border border-emerald-500/30">
            <div className="flex items-center gap-2 mb-1">
              {gridHoverInfo.properties.type === "transmission" ? (
              ...
              <div className="flex items-center gap-1.5 overflow-hidden">
                <span className="text-xs font-bold text-white truncate">
                  {gridHoverInfo.properties.name}
                </span>
                {gridHoverInfo.properties.id && (
                  <span className="text-[9px] bg-white/10 px-1 rounded text-slate-400 font-mono">
                    #{gridHoverInfo.properties.id}
                  </span>
                )}
              </div>
            ...
            <div className="space-y-1.5 mt-2">
              {gridHoverInfo.properties.voltage_kv && (
                <div className="flex justify-between text-[10px]">
                  ...
                  <span className="text-white font-mono">
                    {gridHoverInfo.properties.voltage_kv} kV
                  </span>
                </div>
              )}
              {gridHoverInfo.properties.capacity_mw && (
                <div className="flex justify-between text-[10px]">
                  ...
                  <span className="text-emerald-400 font-bold">
                    {gridHoverInfo.properties.capacity_mw} MW
                  </span>
                </div>
              )}
              ...
              {gridHoverInfo.properties.status && (
                <div className="flex justify-between text-[10px]">
                  ...
                  <span
                    className={`font-bold ${gridHoverInfo.properties.status.includes("Bottleneck") ? "text-rose-400" : "text-emerald-400"}`}
                  >
              ...
              {gridHoverInfo.properties.assets && (
                <div className="pt-1 border-t border-white/5">
                  ...
                  {Object.entries(
                    typeof gridHoverInfo.properties.assets === "string"
                      ? JSON.parse(gridHoverInfo.properties.assets)
                      : gridHoverInfo.properties.assets,
                  ).map(([k, v]: [any, any]) => (
                    <div
                      key={k}
