...
const UnifiedMapPage = () => {
  ...
  const leafletZoom: number = savedLeafletView?.zoom ?? 12;
    ...
    useEffect(() => {
        // configureLeafletIcons(); // Already handled above
    }, []);
        ...
        return (
            <div className="h-screen w-full relative bg-slate-950">
                ...
                <div className="absolute top-20 left-4 z-[9998] flex flex-col gap-2">
                    <button
                        onClick={() => {}}
                        className={`px-3 py-2 rounded-lg text-xs font-bold border backdrop-blur-xl transition-all bg-slate-900/80 text-slate-400 border-white/10`}
                    >
                        📡 {meters.length} meters (stream)
                    </button>
                    <button
